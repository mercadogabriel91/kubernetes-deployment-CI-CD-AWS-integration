#!/bin/bash
# deploy-local-only.sh - Deploy to local Kubernetes only (skip Terraform)
# Use this when AWS infrastructure is already deployed
...
echo "🚀 Local Kubernetes Deployment (Skip AWS Infrastructure)"
echo "========================================================"
echo ""
echo "This script will:"
echo "1. Check/start local Kubernetes (minikube or kind)"
echo "2. Build and push Docker images to ECR (if needed)"
echo "3. Create Kubernetes secrets (ECR auth, AWS credentials)"
echo "4. Deploy Kubernetes manifests"
echo "5. Wait for pods to be ready"
echo "6. Run basic health checks"
echo ""
echo "⚠️  Assumes AWS infrastructure is already deployed!"
echo ""
...
# Step 2: Verify ECR repositories exist
echo -e "${YELLOW}Step 2: Verifying ECR repositories exist...${NC}"
if aws ecr describe-repositories --repository-names k-challenge-dev-auxiliary-service --region $AWS_REGION >/dev/null 2>&1 && \
   aws ecr describe-repositories --repository-names k-challenge-dev-main-api --region $AWS_REGION >/dev/null 2>&1; then
    echo -e "${GREEN}✓ ECR repositories found${NC}"
else
    echo -e "${RED}Error: ECR repositories not found${NC}"
...
# Step 4: Ask if images need to be rebuilt
echo -e "${BLUE}Do you want to rebuild and push Docker images?${NC}"
read -p "Rebuild images? (Y/n): " REBUILD_IMAGES
REBUILD_IMAGES=$(echo "$REBUILD_IMAGES" | tr '[:upper:]' '[:lower:]')

if [ "$REBUILD_IMAGES" != "n" ] && [ "$REBUILD_IMAGES" != "no" ]; then
    ...
    echo -e "${BLUE}Skipping image build/push (using existing images)${NC}"
fi
echo ""
...
echo "  - Using existing AWS Infrastructure: S3, Parameter Store, ECR, IAM"
echo ""
echo "You can:"
